    ...
    try {
      ...
      if (settings.verification.type === 'reaction' && settings.verification.messageId === message.id) {
        ...
        if (isMatch) {
          ...
          if (!role) {
            console.warn(`[Bot Event Warning] Verification role ${roleId} not found in guild ${guildId}`);
            return;
          }
          ...
          if (!member.roles.cache.has(roleId)) {
            ...
            console.log(`[Bot Event] Reaction Verification: Granted role ${role.name} to ${user.username} in guild ${guildId}`);
          }
        }
      }
    } catch (error) {
      console.error('[Bot Event Error] Error in messageReactionAdd handler:', error);
    }
  }
};
